...
function MadNLP.transfer!(
    dest::rocSPARSE.ROCSparseMatrixCSC{T},
    ...
    if length(map) > 0
        backend = ROCBackend()
        MadNLPGPU._transfer_to_map_kernel!(backend)(dest.nzVal, map, src.V; ndrange=length(map))
        synchronize(backend)
    end
    return
end
...
function build_csc_transfer_map(csc_map::ROCVector)
    zvals = ROCVector{Int}(1:length(csc_map))
